...
neuron_num_set=[1000,10000,100000,400000]
...
VarR['s_gam1'] =np.linspace(0.85,1.15,num=7)
VarR['isfail'] = False
...
for gam1 in VarR['s_gam1']:
    ...
    for neu_num in neuron_num_set:
        ...
        while VarR['trysmalllr']:
            ...
            R['output_dim']=1
            R['ActFuc']=1   ###  0: ReLU; 1: Tanh; 3:sin;4: x**5,, 5: sigmoid  6 sigmoid derivate
            R['opti']='1'   ###  0: gd; 1: adam;  
            #if sys.argv[1:]:
        ...
        #    R['learning_rate']=10000
            ...
            BaseDir_a='%s/gamma%s%s/'%(BaseDir,sds,abs(R['gam2']))
            mkdir(BaseDir_a)
            BaseDir_b='%s/sf%sf%s/'%(BaseDir_a,R['beta1exp'],R['beta2exp'])
            mkdir(BaseDir_b)
            BaseDir2='%s/%.3ff%sf%s/'%(BaseDir_b,R['alphaexp'],R['beta1exp'],R['beta2exp'])
            mkdir(BaseDir2)
            subFolderName = '%sr%s'%(R['hidden_units'][0],int(np.absolute(np.random.normal([1])*1000))//int(1)) 
            ...
            plotepoch=12500
            R['train_size']=4;  ### training size
            R['batch_size']=R['train_size'] # int(np.floor(R['train_size'])) ### batch size
            R['test_size']=1000  ### test size
            R['x_start']=-1/2  #math.pi*3 ### start point of input
            R['x_end']=1/2  #6.28/4 #math.pi*3  ### end point of input
            ...
            print(R) 
#            if R['input_dim']==1:
#                R['test_inputs'] =np.reshape(np.linspace(R['x_start'], R['x_end'], num=R['test_size'],
...
#            test_inputs=R['test_inputs']
            R['train_inputs']=[[-0.5],[0],[0.5]]
            train_inputs=R['train_inputs']
#            R['y_true_test']= get_y_func(test_inputs)
            R['y_true_train']=[[-0.5],[0],[1]]
            R['test_inputs']=np.reshape(np.linspace(R['x_start'], R['x_end'], num=R['test_size'], endpoint=True),[R['test_size'],1])
            test_inputs=R['test_inputs']
            #plt.figure()
            #plt.plot(test_inputs,R['y_true_test'])
...
#                    R['loss_test'].append(loss_test_tmp)
                ...
                def wb2theta(self,w_Univ_tmp,b_Univ_tmp):
                    ...
                    amp_av=a*self.L2norm(v,0)
                    angle_v=[math.atan2(dy,dx)  for dy,dx in zip(b_Univ_tmp[0],w_Univ_tmp[0][0])]
